...
async function handleSubmit() {
    ...
    try {
        ...
        showToast(data.message || (isLogin.value ? 'Login successful' : 'Registered successfully'), 'success')
        // console.log(data)
    } catch (err) {
        console.error(err)
  ...
  if (isLogin.value) {
    alert(`Logging in: ${form.value.username}`)
    // TODO: Add actual login logic
  } else {
    alert(`Registering student: ${form.value.name}`)
